...
| "invar' a k (Node l p r) \<longleftrightarrow> dim p = k \<and>
...
| "ins_kdt' a k p (Node l p' r) = (
    if p = p' then Node l p' r
      ...
      let k' = incr a k in
      if p!a \<le> p'!a then
        Node (ins_kdt' k' k p l) p' r
      else
        Node l p' (ins_kdt' k' k p r)
  )"
...
lemma invar_ins_kdt': 
  ...
  using assms by (induction kdt arbitrary: a) (auto simp add: set_ins_kdt' Let_def)
...
| "query_area' a k p\<^sub>0 p\<^sub>1 (Node l p r) = (
    ...
    if point_in_bounding_box k p p\<^sub>0 p\<^sub>1 then
      {p} \<union> query_area' k' k p\<^sub>0 p\<^sub>1 l \<union> query_area' k' k p\<^sub>0 p\<^sub>1 r
    else
      if p!a < p\<^sub>0!a then
        query_area' k' k p\<^sub>0 p\<^sub>1 r
      else if p!a > p\<^sub>1!a then
        query_area' k' k p\<^sub>0 p\<^sub>1 l
      else
        query_area' k' k p\<^sub>0 p\<^sub>1 l \<union> query_area' k' k p\<^sub>0 p\<^sub>1 r
  )"
