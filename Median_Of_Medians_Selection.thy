...
proof -
  ...
  text \<open>The medians of the sublists\<close>
  define M where "M = mset (map med (chop d xs))"
  define YS where "YS = mset (chop d xs)"
  text \<open>The sublists with a smaller median than the median-of-medians @{term x} and the rest.\<close>
  ...
  text \<open>At most roughly half of the lists have a median that is smaller than @{term M}\<close>
  ...
  text \<open>We estimate the number of elements less than @{term x} by grouping them into elements
  ...
  text \<open>We do some further straightforward estimations and arrive at our goal.\<close>
